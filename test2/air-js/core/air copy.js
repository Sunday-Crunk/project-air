...
class ReactiveState {
  constructor(initialValue) {
    ...
    this.read = (help) => {
      ...
      console.log("reading value: ", value, help, currentComponent)
      if (effect) {
        console.log("effect: ", effect)
        subscribers.add(effect);
        console.log("subscribers: ", subscribers)
      }else{
        console.log("no effect: ", currentComponent)
      }
      return value;
    }; 
      ...
      if (newValue !== value) {
        value = newValue;
          ...
          currentComponent.renderManager.addPrimaryRender(() => {
            console.log("scheduled subscribers: ", subscribers)
            
            subscribers.forEach(sub => {
              console.log("suub: ", sub, value)
              sub(value)
            });
          });
          ...
          currentComponent.renderManager.addSecondaryRender(() => {
            console.log("scheduled secondary subscribers: ", subscribers)
            onUpdateSubscribers.forEach(sub => {
              console.log("onUpdate subscriber: ", sub)
              sub(value)
            });
          });
    ...
    connectedCallback() {
      ...
      const cleanup = createEffect(() => {
        ...
        if (this.previousVdom){
          console.log("updating element: ", this, processedTemplate)
      ...
      return strings.reduce((result, string, i) => {
        ...
        if (i < values.length) {
            ...
            if (isReactiveState(values[i])) {
              ...
              console.log("comp props: ", values[i])
              console.log("string: ", string, strings[i-1])
              if (!string.includes("<")){
...
function extractAttribute(str, attribute) {
  console.log("extracting attribute: ", attribute)
  console.log("str: ", str)
  const parts = str.split(/<(?!.*<)/); // Split at the last occurrence of <
  console.log("parts: ", parts)
  const afterLastTag = parts[parts.length] || '';
  ...
  const parse = () => {
    ...
    const processPsuedoElement = (group, properties) =>{
      ...
      if (styleRules.length>0){
        ...
        const cssString = generatePseudoElementCSS(styleRules, psuedoId,  group.replace("_",""));
